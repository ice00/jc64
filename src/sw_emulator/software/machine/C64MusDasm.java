        ...
        switch (b1) {
          ...
          case 0x01:                        // look for volume
            switch (b2) {
              ...
              case 0x67: result="WAVEFORM:SET 3=TS"; break;
              case 0x87: result="WAVEFORM:SET 4=P  (PULSE)"; break;
              case 0xA7: result="WAVEFORM:SET 5=TP"; break;
              case 0xC7: result="WAVEFORM:SET 6=SP"; break;
              case 0xE7: result="WAVEFORM:SET 7=TSP"; break;
          ...
          case 0xA6:
            result="FREQ:TRANSPOSE (#value not yet calculated#)";
            break;

          case 0x2E:
            result="FREQ:RELATIVE TPS (#value not yet calculated#)";
            break;

          default:                          // look for all the other cases
            if ((b1 & 0x0F)==0x02) {
              result="WAVEFORM:PULSE WIDTH "+ ((b1 & 0xF0)<<4+b2);
            }

            if ((b1 & 0x03)==0x03) {
              result="FREQ:PORTAMENTO "+ ((b1 & 0xFC)<<6+b2);
            }

            if ((b1 & 0x1F)==0x1A) {
              result="FREQ:DETUNE "+(((b1 & 0xE0)<<3+b2)-2048);
            }

            if ((b1 & 0x1F)==0x0A) {
              result="FREQ:DETUNE "+((b1 & 0xE0)<<3+b2);
            }

            if ((b1 & 0x30)==0x30) {
              result="MISC:JIFFY LENGTH "+((b1 & 0xC0)<<2+b2-200);
            }
        }
      }
    this.pos+=2;
